...
export function Rutas() {
  ...
  useEffect(() => {
    supabase().from("carretillas").select("id, codigo, estado").then(({ data }) => {
      setCarretillas((data ?? []) as Carretilla[]);
    });
    supabase().from("usuarios").select("id, nombre").then(({ data }) => {
  ...
  const verRuta = async (j: Jornada, idx: number) => {
    ...
    setRutaSel({
      ...
      puntos: (ubis ?? []).map((u: any) => [Number(u.latitud), Number(u.longitud)]),
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid md:grid-cols-3 gap-4">
          ...
          <div className="space-y-1 max-h-64 overflow-y-auto">
            {carretillas.map((c) => (
              <button key={c.id}
                ...
                className={`w-full text-left px-3 py-2 rounded-lg flex justify-between items-center ${carretillaId === c.id ? "bg-[#AEE6F9]" : "hover:bg-gray-50"}`}>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-4 space-y-3">
          ...
          <div className="space-y-1 max-h-48 overflow-y-auto">
            {jornadas.length === 0 && carretillaId && <div className="text-sm text-gray-500">Sin jornadas en esta fecha</div>}
            {jornadas.map((j, i) => (
              <button key={j.id} onClick={() => verRuta(j, i)}
                className={`w-full text-left px-3 py-2 rounded-lg text-sm ${rutaSel?.id === j.id ? "bg-[#F8C8DC]" : "hover:bg-gray-50"}`}>
                <div className="font-medium">{trabajadores[j.trabajador_id] ?? "—"}</div>
                <div className="text-xs text-gray-500">
                  {new Date(j.hora_inicio).toLocaleTimeString()} →{" "}
                  {j.hora_fin ? new Date(j.hora_fin).toLocaleTimeString() : "—"} · {j.estado}
                </div>
              </button>
            ))}
          </div>
        ...
        <div className="md:col-span-1 bg-white rounded-2xl border border-gray-200 p-4">
          ...
          {rutaSel ? (
            <div className="text-sm space-y-1">
              <div><strong>Carretilla:</strong> {rutaSel.carretillaCodigo}</div>
              <div><strong>Trabajador:</strong> {rutaSel.trabajadorNombre}</div>
              <div><strong>Estado:</strong> {rutaSel.estado}</div>
              <div><strong>Inicio:</strong> {rutaSel.horaInicio ? new Date(rutaSel.horaInicio).toLocaleString() : "—"}</div>
              <div><strong>Fin:</strong> {rutaSel.horaFin ? new Date(rutaSel.horaFin).toLocaleString() : "—"}</div>
              <div><strong>Puntos GPS:</strong> {rutaSel.puntos.length}</div>
            </div>
          ) : (
      ...
      {rutaSel && rutaSel.puntos.length > 0 && (
        <div style={{ height: 500 }}>
          <MapView rutas={[rutaSel]} />
        </div>
      )}
    </div>
  );
